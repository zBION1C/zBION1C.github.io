				...
				children: [{
					...
					children: [{
						...
						children: query.completions.filter((i) => i.name.startsWith(query.completionsPrefix || "____")).map((i) => ({
							type: "element",
							tagName: "li",
							properties: { class: "kindModifiers" in i && typeof i.kindModifiers === "string" && i.kindModifiers?.split(",").includes("deprecated") ? "deprecated" : void 0 },
							children: [{
								type: "element",
								tagName: "span",
								properties: {},
...
function rendererRich(options = {}) {
	const { completionIcons = defaultCompletionIcons, customTagIcons = defaultCustomTagIcons, processHoverInfo = defaultHoverInfoProcessor, processHoverDocs = (docs) => docs, classExtra = "", jsdoc = true, errorRendering = "line", queryRendering = "popup", renderMarkdown = renderMarkdownPassThrough, renderMarkdownInline = renderMarkdownPassThrough, hast } = options;
	function highlightPopupContent(info) {
		...
		if (jsdoc && info.tags?.length) popupContents.push(extend(hast?.popupDocsTags, {
			type: "element",
			tagName: "div",
			properties: { class: "twoslash-popup-docs twoslash-popup-docs-tags" },
			children: info.tags.map((tag) => ({
				type: "element",
				tagName: "span",
				properties: { class: `twoslash-popup-docs-tag` },
				children: [{
					type: "element",
					tagName: "span",
					properties: { class: "twoslash-popup-docs-tag-name" },
					children: [{
						type: "text",
						value: `@${tag[0]}`
					}]
					...
					type: "element",
					tagName: "span",
					properties: { class: "twoslash-popup-docs-tag-value" },
	...
	return {
		nodeStaticInfo(info, node) {
			...
			const popup = extend(hast?.hoverPopup, {
				type: "element",
				tagName: "span",
				properties: { class: ["twoslash-popup-container", classExtra].filter(Boolean).join(" ") },
			...
			return extend(hast?.hoverToken, {
				...
				children: hast?.hoverCompose ? hast?.hoverCompose({
					popup,
					token: node
				}) : [popup, node]
			});
		...
		nodeQuery(query, node) {
			...
			if (queryRendering !== "popup") return extend(hast?.queryToken, {
				type: "element",
				tagName: "span",
				properties: { class: "twoslash-hover" },
				children: [node]
			...
			return extend(hast?.queryToken, {
				type: "element",
				tagName: "span",
				properties: { class: "twoslash-hover twoslash-query-persisted" },
				children: hast?.queryCompose ? hast?.queryCompose({
					popup,
					token: node
				}) : [popup, node]
			});
		...
		nodeCompletion(query, node) {
			if (node.type !== "text") throw new ShikiTwoslashError(`Renderer hook nodeCompletion only works on text nodes, got ${node.type}`);
			const items = query.completions.map((i) => {
				...
				return {
					...
					children: [...completionIcons ? [{
						...
						type: "element",
						tagName: "span",
						properties: { class: isDeprecated ? "deprecated" : void 0 },
						children: [{
							type: "element",
							tagName: "span",
			...
			const cursor = extend(hast?.completionCursor, {
				type: "element",
				tagName: "span",
				properties: { class: ["twoslash-completion-cursor", classExtra].filter(Boolean).join(" ") },
				children: []
			});
			const popup = extend(hast?.completionPopup, {
				type: "element",
				tagName: "ul",
				properties: { class: ["twoslash-completion-list", classExtra].filter(Boolean).join(" ") },
				children: items
			});
			const children = [];
			if (node.value) children.push({
				type: "text",
				value: node.value
			});
			if (hast?.completionCompose) children.push(...hast.completionCompose({
				popup,
				cursor
			}));
			else children.push({
		...
		nodesError(error, children) {
			if (errorRendering !== "hover") return [extend(hast?.errorToken, {
				type: "element",
				tagName: "span",
				properties: { class: [`twoslash-error`, getErrorLevelClass(error)].filter(Boolean).join(" ") },
				children
			})];
			const popup = extend(hast?.errorPopup, {
				...
				tagName: "span",
				properties: { class: ["twoslash-popup-container", classExtra].filter(Boolean).join(" ") },
				children: [extend(hast?.popupError, {
					type: "element",
					tagName: "div",
					properties: { class: "twoslash-popup-error" },
			...
			return [extend(hast?.errorToken, {
				type: "element",
				tagName: "span",
				properties: { class: `twoslash-error twoslash-error-hover ${getErrorLevelClass(error)}` },
				children: hast?.errorCompose ? hast?.errorCompose({
					popup,
					token
				}) : [popup, token]
			})];
		...
		lineQuery(query, node) {
			...
			return [{
				type: "element",
				tagName: "div",
				properties: { class: ["twoslash-meta-line twoslash-query-line", classExtra].filter(Boolean).join(" ") },
				children: [{
					type: "text",
					value: " ".repeat(offset)
				}, {
		...
		lineCustomTag(tag) {
			return [{
				type: "element",
				tagName: "div",
				properties: { class: [`twoslash-tag-line twoslash-tag-${tag.name}-line`, classExtra].filter(Boolean).join(" ") },
				children: [...customTagIcons ? [{
					type: "element",
					tagName: "span",
		...
		nodesHighlight(highlight, nodes) {
			return [extend(hast?.nodesHighlight, {
				type: "element",
				tagName: "span",
				properties: { class: "twoslash-highlighted" },
				children: nodes
			})];
		}
	};
