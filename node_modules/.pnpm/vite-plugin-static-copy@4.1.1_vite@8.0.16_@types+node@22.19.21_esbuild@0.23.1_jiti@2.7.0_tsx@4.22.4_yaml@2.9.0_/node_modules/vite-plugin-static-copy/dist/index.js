...
function applyRenameObject(renameObj, target, dir) {
	let result = target;
	if ("stripBase" in renameObj && renameObj.stripBase !== void 0) {
	...
	if ("name" in renameObj && renameObj.name !== void 0) {
		const parsed = path.parse(result);
		result = path.join(parsed.dir, renameObj.name);
	}
	return result;
}
async function renameTarget(target, rename, src, dir) {
	if (typeof rename === "string") return rename;
	if (typeof rename === "object") return applyRenameObject(rename, target, dir);
	const parsedPath = path.parse(target);
	const result = await rename(parsedPath.name, parsedPath.ext.replace(".", ""), src);
	if (typeof result === "object") return applyRenameObject(result, target, dir);
	return result;
}
const collectCopyTargets = async (root, targets, silent) => {
	const copyTargets = [];
	for (const target of targets) {
		...
		for (const matchedPath of matchedPaths) {
			...
			let destDir;
			if (!dir) destDir = dest;
			else destDir = path.join(dest, dirClean);
			copyTargets.push({
				src: relativeMatchedPath,
				dest: path.join(destDir, rename ? await renameTarget(base, rename, absoluteMatchedPath, dirClean) : base),
				transform,
...
const copyAll = async (rootSrc, rootDest, targets, silent) => {
...
//#endregion
//#region src/serve.ts
const servePlugin = ({ targets, watch, silent, environment }) => {
	let config;
	let watcher;
	...
	const collectFileMap = async () => {
		...
		try {
			updateFileMapFromTargets(await collectCopyTargets(config.root, targets, silent), fileMap, absoluteBuildOutDir);
		} catch (e) {
	...
	return {
		...
		configureServer({ httpServer, middlewares, ws }) {
			...
			watcher.on("add", async (path) => {
				if (!silent) config.logger.info(formatConsole(`${pc.green("detected new file")} ${path}`), { timestamp: true });
				await collectFileMapDebounce();
				if (watch.reloadPageOnChange) reloadPage();
			});
			if (watch.reloadPageOnChange) {
				watcher.on("change", (path) => {
					if (!silent) config.logger.info(formatConsole(`${pc.green("file changed")} ${path}`), { timestamp: true });
					reloadPage();
				});
				watcher.on("unlink", (path) => {
					if (!silent) config.logger.info(formatConsole(`${pc.green("file deleted")} ${path}`), { timestamp: true });
					reloadPage();
				});
			}
