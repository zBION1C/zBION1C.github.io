...
function buildRemoteUrl(baseUrl, payload, transport) {
	...
	if (transport === "fragment") {
		...
		if (!rawHash) return `${before}#${param}`;
		...
		function createChildProcess() {
			...
			(async () => {
				try {
					...
					if (currentRun === runId) errorStream(error);
				}
			})();
			return cp;
		}
