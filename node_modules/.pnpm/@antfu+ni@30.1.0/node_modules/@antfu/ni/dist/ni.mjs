...
import { A as require_prompts, T as formatPackageWithUrl, h as getCatalog, i as runCli, l as parseNi, o as getCommand, w as exclude } from "./src-Dc2SIy_D.mjs";
...
function updatePackageJsonCatalogRefs(pkgJsonPath, entries, depType) {
	...
	const sorted = {};
	for (const key of Object.keys(data[depType]).sort((a, b) => a.localeCompare(b))) sorted[key] = data[depType][key];
	data[depType] = sorted;
...
async function promptSelectCatalog(config, pkgName, programmatic) {
	...
	const { catalog } = await (0, import_prompts.default)({
		...
		choices: [
			...catalogChoices,
			{
				title: styleText("dim", "create new catalog"),
				value: CREATE_NEW
			},
			{
				title: styleText("dim", "skip (install without catalog)"),
				value: SKIP
			}
		]
	});
	...
	for (const pkg of packages) {
		...
		if (result.catalogName) {
			if (result.version) {
				...
				if (!ctx?.programmatic) console.log(`${styleText("green", "+")} ${styleText("cyan", pkg)} ${styleText("dim", `→ ${result.catalogName} catalog (${result.version})`)}`);
...
async function fetchNpmPackages(pattern) {
	...
	try {
		return (await fetch(registryLink(pattern)).then((res) => res.json())).objects.map(({ package: pkg }) => ({
			title: formatPackageWithUrl(`${pkg.name.padEnd(30, " ")} ${styleText("blue", `v${pkg.version}`)}`, pkg.links.repository ?? pkg.links.npm, terminalColumns),
			value: pkg
		}));
	} catch {
