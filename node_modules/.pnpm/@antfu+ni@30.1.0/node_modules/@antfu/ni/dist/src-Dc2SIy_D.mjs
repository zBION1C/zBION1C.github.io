...
function formatPackageWithUrl(pkg, url, limits = 80) {
	return url ? terminalLink(pkg, url, { fallback: (_, url) => pkg.length + url.length > limits ? pkg : `${pkg} ${styleText("dim", `- ${url}`)}` }) : pkg;
}
//#endregion
...
async function runCli(fn, options = {}) {
	...
	try {
		...
		if (error instanceof UnsupportedCommand && !options.programmatic) console.log(styleText("red", `\u2717 ${error.message}`));
...
async function run(fn, args, options = {}) {
	...
	if (args.length === 1 && (args[0]?.toLowerCase() === "-v" || args[0] === "--version")) {
		...
		if (agent) console.log(`${agent.padEnd(10)} ${styleText("blue", agentVersion)}`);
	...
	if (args.length === 1 && args[0] === "--agent") {
		...
		else process$1.stdout.write("unknown\n");
		return;
	}
