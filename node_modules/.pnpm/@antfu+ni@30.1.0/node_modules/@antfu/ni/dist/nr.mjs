...
import { A as require_prompts, E as limitText, S as CLI_TEMP_DIR, d as parseNr, g as getConfig, i as runCli, k as writeFileSafe } from "./src-Dc2SIy_D.mjs";
...
runCli(async (agent, args, ctx) => {
	...
	const promptSelectScript = async (raw) => {
		...
		const choices = raw.reduce((acc, { key, description }) => {
			...
			if (!noLastCommand && last && key === last) return [item, ...acc];
			return [...acc, item];
		}, []);
