...
function preprocessHead(frontmatter, options) {
	...
	if (head.title) {
		if (!meta.some((i) => i.property === "og:title")) meta.push({
			property: "og:title",
			content: head.title
		});
		if (!meta.some((i) => i.name === "twitter:title")) meta.push({
			name: "twitter:title",
			content: head.title
		});
	}
	if (head.description) {
		if (!meta.some((i) => i.name === "description")) meta.push({
			name: "description",
			content: head.description
		});
		if (!meta.some((i) => i.property === "og:description")) meta.push({
			property: "og:description",
			content: head.description
		});
		if (!meta.some((i) => i.name === "twitter:description")) meta.push({
			name: "twitter:description",
			content: head.description
		});
	}
	if (head.image) {
		if (!meta.some((i) => i.property === "og:image")) meta.push({
			property: "og:image",
			content: head.image
		});
		if (!meta.some((i) => i.name === "twitter:image")) meta.push({
			name: "twitter:image",
			content: head.image
		});
		if (!meta.some((i) => i.property === "twitter:card")) meta.push({
			name: "twitter:card",
			content: "summary_large_image"
		});
	}
	const result = {};
	for (const [key, value] of Object.entries(head)) if (headProperties.includes(key)) result[key] = value;
...
function extractCustomBlock(html, options) {
	const blocks = [];
	for (const tag of options.customSfcBlocks) html = html.replace(new RegExp(`<${tag}[^>]*\\b[^>]*>[^<]*<\\/${tag}>`, "gm"), (code) => {
		blocks.push(code);
		return "";
	});
	return {
...
function createMarkdown(options) {
	...
	const setupPromise = (async () => {
		...
		if (options.frontmatter || options.excerpt) md.use(frontmatterPlugin, {
	...
	return async (id, raw) => {
		...
		const { wrapperDiv, wrapperClasses, wrapperComponent, transforms, headEnabled, frontmatterPreprocess } = options;
		raw = raw.trimStart();
		...
		if (wrapperDiv) {
			const wrapperClassesResolved = toArray(typeof wrapperClasses === "function" ? wrapperClasses(id, raw) : wrapperClasses).filter(Boolean).join(" ");
			if (wrapperClassesResolved) html = `<div class="${wrapperClassesResolved}">${html}</div>`;
		...
		if (wrapperComponentName) html = `<${wrapperComponentName} ${[options.frontmatter && ":frontmatter=\"frontmatter\"", options.excerpt && ":excerpt=\"excerpt\""].filter(Boolean).join(" ")}>${html}</${wrapperComponentName}>`;
		html = await transforms.after?.(html, id) ?? html;
		if (options.escapeCodeTagInterpolation) html = html.replace(/<code(.*?)>/g, "<code$1 v-pre>");
		...
		if (options.frontmatter) {
			if (options.excerpt && data) {
				if (data.excerpt !== void 0) excerptKeyOverlapping = true;
				data.excerpt = excerpt;
			}
			const { head, frontmatter } = frontmatterPreprocess(data || {}, options, id, preprocessHead);
			if (options.excerpt && !excerptKeyOverlapping && frontmatter.excerpt !== void 0) delete frontmatter.excerpt;
			scriptLines.push(`const frontmatter = ${JSON.stringify(frontmatter)}`);
			if (options.exportFrontmatter) frontmatterExportsLines = Object.entries(frontmatter).map(([key, value]) => {
				if (EXPORTS_KEYWORDS.includes(key)) key = `_${key}`;
				return `export const ${key} = ${JSON.stringify(value)}`;
			});
			if (!isVue2 && options.exposeFrontmatter && !hasExplicitExports()) scriptLines.push("defineExpose({ frontmatter })");
			if (!isVue2 && headEnabled && head) {
				if (headEnabled === "vueuse") throw new Error("unplugin-vue-markdown no longer supports @vueuse/head. Change `headEnabled` to `true` and install `@unhead/vue` instead.");
		...
		if (options.excerpt) {
			...
			if (!isVue2 && options.exposeExcerpt && !hasExplicitExports()) scriptLines.push("defineExpose({ excerpt })");
		}
...
const _require = typeof __require === "undefined" ? createRequire(import.meta.url) : __require;
...
const unpluginFactory = (userOptions = {}) => {
	const markdownToVue = createMarkdown(resolveOptions(userOptions));
	const filter = createFilter(userOptions.include || /\.md$|\.md\?vue/, userOptions.exclude || cssIdRE);
	return {
...
//#endregion
export { unpluginFactory as n, src_default as t };
