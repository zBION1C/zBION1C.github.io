...
class Simulator {
  constructor(ctx,systems, attacks, success, failure, p, freq, x, y, mid_day){
    ...
    //this.box.drawLine(0, this.upper*4, this.box.w, this.upper, "Black")
    //this.box.drawLine(0, this.lower*4, this.box.w, this.lower, "Black")
  ...
  async simulate() {
    ...
    for (let i = 1; i <= this.systems; i++){
      ...
      for (let j = 1; j <= this.attacks; j++) {
        ...
        if (j == this.mid_day){
          this.histo1.addToBin(cdown/i * 100)
        }
        console.log(cdown/i * 100)
      ...
      this.histo2.addToBin(cdown/i * 100);
    }
  }
}
  ...
  const s = new Simulator(ctx, systems, attacks, 1, -1, 0.5, "", 0, 100, 20);
  s.simulate();
